...
export const createUserName = async (userId, userName) => {
  try {
    const match = await getUser(userName);
...
export const getUserRecordings = async (userId) => {
  ...
  const userRecordings = user.recordings.map((id) => getRecording(id));

  return Promise.all(userRecordings);
};
...
export const createRecording = async ({
  sessionId, title, description, username, tags, userIds, photoId,
}) => {
  const db = FirebaseConfig.firestore();
  const Recordings = db.collection('recordings');
  const Users = db.collection('users');
